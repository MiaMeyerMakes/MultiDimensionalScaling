...
bio <- read.csv("bioenv.csv", header=TRUE)
bio$Sediment <- as.factor(bio$Sediment)
abio <- bio[,7:10]
cbio <- bio[,2:10]
```
...
### (a) Obtain a classical scaling MDS display of the sea bed sites in the bioenv.data using the environmental variables for calculating distances with the Gower metric.

```{r}
# Get the Gower distances
gowerDist <- gower.dist(abio)

# Obtain classical scaling MDS display
bioMDS <- cmdscale(gowerDist)
mds_df <- as.data.frame(bioMDS)

# Create the plot using ggplot2
ggplot(mds_df, aes(x = V1, y = V2)) +
  geom_point(shape = 16, col = "mediumturquoise") +
  labs(x = "X", y = "Y", title = "Classical scaling of 'bioenv' data with Gower distance and k=2")

coordsTable <- data.frame(X_coords = bioMDS[,1], Y_coords = bioMDS[,2])

myTable = knitr::kable(coordsTable, col.names=c("X value","Y value"), caption="2-dimensional scaling of 'bioenv' data",escape=FALSE, digits=4)
myTable %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```

### (b) Repeat (a) but add the information of the continuous environmental variables in the form of calibrated linear axes.

```{r}
reg1 <- Regression.biplot(abio[,1:3], bioMDS, dim.biplot = 2)
```

### (c) Repeat (b) but add information of the five groups of species variables in the form of calibrated linear axes.

```{r}
reg2 <- Regression.biplot(cbio[,1:8], bioMDS, dim.biplot = 2)
```

## Assignment 7.2

### (a) Write an R function to implement Guttman's algorithm. Provide for all possible permutations of the elements of $\mathit{\mathbf{x}}$ when $n\le8$ and a random selection of the total number of permutations when $n>8$.

```{r, collectcode = TRUE}
# the function to call which calculates the newest permutation for x
myperm <- function(Rrow = R[1,], delta=platodelta, n=7, maxiter=20)
{
  stress <- numeric(maxiter)
  x = Rrow
  xnew = matrix(0, ncol = maxiter, nrow = n)
  xnew[,1] <- x
  ...
  for (j in 1:(maxiter-1))
  {
      ...
      for (r in 1:n)
      {
        diff = delta[r,]*sign(xnew[r,j]-xnew[,j]) # the value inside the sum
        xnew[r,j+1] <- sum(diff)/n
      }
      diffmat <- matrix(rep(xnew[,j+1],n), ncol =n, byrow=TRUE)-matrix(rep(xnew[,j+1],n), ncol =n)
      stress[j+1] <- (sum( (delta-abs(diffmat))^2 ))/2
  }
  return(list(stress = tail(stress,1), x=xnew[,n]))
}

# Actual function implementing Guttman's algorithm
myGuttman <- function(X, delta, maxiter = 20)
{
  Xc <- X-mean(X) # centered x
  n <- length(Xc)
  
  if (n>8) # algorithm for n>8
  {
    # generate 500 permutations
    permutations <- t(replicate(4000, sample(Xc, size = n)))
    # convert to matrix + remove duplicate rows
    R <- unique(permutations)
    
  } else { # algorithm for n≤8
    # get all the permutations
    R <- matrix(unlist(permn(Xc)), ncol = n, byrow=TRUE)
  }
  
  nR <- nrow(R)
  x_results <- matrix(0, nrow=n, ncol = nR)
  stresses <- numeric(nR)
  # now for every permutation in R:
  for (l in 1:nR)
  {
    result = myperm(Rrow = R[l,], delta=delta, n=n, maxiter=maxiter)
    stresses[l] <- result$stress
    x_results[,l] <- result$x
  }
  
  mymin <- which.min(stresses)
  return(list(x = x_results[,mymin], stress = min(stresses)))
}
```

### (b) Apply this function to the Plato dataset

```{r, collectcode = TRUE}
platodelta <- matrix(c(00.00, 12.28, 10.72, 12.65, 11.55, 8.59, 8.89,
                       12.28, 00.00, 11.80, 6.44, 8.58, 10.58, 13.02,
                       10.72, 11.80, 00.00, 11.75, 11.96, 9.38, 9.52,
                       12.65, 6.44, 11.75, 00.00, 6.99, 10.08, 12.26,
                       11.55, 8.58, 11.96, 6.99, 00.00, 7.72, 11.23,
                       8.59, 10.58, 9.38, 10.08, 7.72, 00.00, 6.10,
                       8.89, 13.02, 9.52, 12.26, 11.23, 6.10, 00.00), 
                        byrow=TRUE, ncol = 7)
...
Xresults <- myGuttman(X = -3:3, delta = platodelta)
dissimMat <- data.frame(platodelta-as.matrix(dist(Xresults$x)))
row.names(dissimMat) <- PlatoNames
finalX <- data.frame(PlatoNames, Xresults$x)

Xresults$stress

myTable = knitr::kable(finalX, col.names=c("Name","X configuration"), caption="Final configuration of Guttman's algorithm",escape=FALSE, digits=4)
myTable %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))

myTable2 = knitr::kable(dissimMat, col.names=PlatoNames, caption="The 'errors': delta-distance",escape=FALSE, digits=4)
myTable2 %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
Since we know that "Rep" is first and "Laws" is last, we can use this information when performing unidimensional scaling.
...
R_inside <- matrix(unlist(permn(Xc[2:6])), ncol = n-2, byrow=TRUE)
R <- cbind(-3,R_inside,3)
...
Xresults <- list(x = x_results[,mymin], stress = min(stresses))
dissimMat <- data.frame(platodelta-as.matrix(dist(Xresults$x)))
row.names(dissimMat) <- PlatoNames
finalX <- data.frame(PlatoNames, Xresults$x)
